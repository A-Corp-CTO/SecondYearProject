...
def data2feats(inputData, word2idx, label2idx):
    ...
    print(labels)
    # feats_X, labels_Y = smote.fit_resample(feats,labels)
    # feats_1, labels_2 = rus.fit_resample(feats_X,labels_Y)
    return feats, labels

train_feats, train_labels = data2feats(train_data, token_to_id, tag_to_id)
X_res, Y_res = rus.fit_resample(train_feats,train_labels)
print(X_res)
...
def run_eval(feats_batches, labels_batches):
    model.fit_transform(feats_batches,labels_batches)
    predictions = []
